...
import { Bell, User, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
...
const KIND_META: Record<NotifKind, {
  borderColor: string; iconBg: string; Icon: React.ElementType; iconColor: string;
}> = {
  expiring_soon:   { borderColor: "border-l-red-500",     iconBg: "bg-red-100",     Icon: AlertTriangle, iconColor: "text-red-500"  },
  expired:         { borderColor: "border-l-red-400",     iconBg: "bg-red-50",      Icon: XCircle,       iconColor: "text-red-400"  },
  receipt_processed: { borderColor: "border-l-primary/30", iconBg: "bg-primary/10", Icon: CheckCircle2,  iconColor: "text-primary"  },
};
  ...
  return (
    ...
    >
      <div className="flex gap-3">
        <div className={`w-10 h-10 rounded-xl ${meta.iconBg} flex items-center justify-center shrink-0 mt-0.5`}>
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          <div className="flex items-start justify-between gap-2">
            <p className="font-semibold text-sm leading-tight">{
              item.kind === "expiring_soon"    ? "Warranty Expiring Soon" :
              item.kind === "expired"          ? "Warranty Expired"       :
                                                 "Receipt Processed"
            }</p>
          ...
          <p className="text-sm text-foreground/80 leading-snug">
            {item.kind === "expiring_soon" && (
              <>Your <strong>{item.productName}</strong> warranty expires in{" "}
                <strong className="text-red-500">{item.daysLeft} day{item.daysLeft !== 1 ? "s" : ""}</strong>.{" "}
                Tap to view details and options for extension.</>
            )}
            {item.kind === "expired" && (
              <>Your <strong>{item.productName}</strong> warranty expired{" "}
                {Math.abs(item.daysLeft)} day{Math.abs(item.daysLeft) !== 1 ? "s" : ""} ago.</>
            )}
            {item.kind === "receipt_processed" && (
              <>Your <strong>{item.productName}</strong> receipt has been successfully scanned and added to your dashboard.</>
            )}
          </p>
