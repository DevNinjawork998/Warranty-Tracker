...
export async function POST(req: Request) {
  try {
    ...
    const blob = await put(`receipts/${session.user.id}/${Date.now()}-${safeName}`, fileBytes, {
      access: "private",
    ...
    const fileContent = isPdf
      ...
      : { type: "image" as const, source: { type: "base64" as const, media_type: detectedMime as "image/jpeg" | "image/png" | "image/gif" | "image/webp", data: base64Data } };
