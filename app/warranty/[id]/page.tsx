...
export default async function WarrantyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const warranty = await db.warranty.findFirst({ where: { id, userId: session.user.id } });
      ...
      <main className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        {status === "expiring_soon" && (
          <div className="flex items-center gap-3 bg-amber-100 border border-amber-300 rounded-2xl px-4 py-3">
            ...
            <div>
              <p className="font-semibold text-amber-800 text-sm">Expiring Soon</p>
        ...
        {status === "expired" && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-2xl px-4 py-3">
            ...
            <div>
              <p className="font-semibold text-red-700 text-sm">Warranty Expired</p>
              <p className="text-xs text-red-600">Expired {format(warranty.expiryDate, "d MMM yyyy")}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-card rounded-2xl border p-5 space-y-3">
          <h2 className="text-xl font-bold text-primary">{warranty.productName}</h2>
          ...
          {warranty.priceMyr && (
            <div className="text-sm">
              ...
              <p className="font-medium">RM {Number(warranty.priceMyr).toFixed(2)}</p>
            </div>
          )}
          {warranty.serialNumber && (
            <div className="text-sm">
              <p className="text-muted-foreground text-xs">Serial Number</p>
              <p className="font-medium font-mono text-xs">{warranty.serialNumber}</p>
            </div>
          )}
        </div>
        ...
        {/* Timeline card */}
        <div className="bg-card rounded-2xl border p-5 space-y-3">
          <p className="text-xs font-bold tracking-widest text-primary uppercase">Timeline</p>
          <div className="space-y-0 divide-y">
            <div className="flex items-center justify-between py-3 text-sm">
              <span className="text-muted-foreground">Purchase Date</span>
              <span className="font-medium">{format(warranty.purchaseDate, "d MMM yyyy")}</span>
            </div>
            <div className="flex items-center justify-between py-3 text-sm">
              <span className="text-muted-foreground">Warranty Period</span>
              <span className="font-medium">{formatWarrantyPeriod(warranty.warrantyMonths)}</span>
            </div>
            <div className="flex items-center justify-between py-3 text-sm">
              <span className="text-muted-foreground">Expiry Date</span>
              <span className={`font-medium ${status !== "active" ? "text-red-500" : ""}`}>
        ...
        {hasReceipt && (
          <section className="space-y-2">
            <p className="text-xs font-bold tracking-widest text-muted-foreground uppercase">Original Receipt</p>
