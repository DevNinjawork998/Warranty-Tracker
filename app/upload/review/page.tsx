...
import { z } from "zod";
import {
  Calendar, Wallet, ShieldCheck, LayoutGrid, Store, Maximize2, Camera,
  BookmarkCheck,
} from "lucide-react";
...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    let data: OcrResult;
    try { data = JSON.parse(raw); } catch { router.push("/upload"); return; }
    setOcr(data);
    ...
    if (data.warranty_months) {
      const closest = WARRANTY_OPTIONS.reduce((prev, cur) =>
        Math.abs(cur.value - data.warranty_months!) < Math.abs(prev.value - data.warranty_months!) ? cur : prev
      );
        ...
        {ocr.receipt_preview && (
          <section className="space-y-2">
            ...
            <div className="relative rounded-2xl overflow-hidden border">
              {isPdf ? (
                <iframe src={ocr.receipt_preview} className="w-full" style={{ height: 260 }} title="Receipt" />
              ) : (
                <div className="relative w-full aspect-[4/3]">
                  <Image src={ocr.receipt_preview} alt="Receipt" fill className="object-contain bg-muted" unoptimized />
                </div>
              )}
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="bg-card rounded-2xl border divide-y">
              ...
              <div className="p-4 space-y-1.5">
                ...
                {errors.warrantyMonths && <p className="text-xs text-destructive">{errors.warrantyMonths.message}</p>}
              </div>
