...
describe("POST /api/warranties — expiryDate computation", () => {
	it("computes expiryDate as purchaseDate + warrantyMonths (12 months)", async () => {
		await POST(makeRequest({ productName: "TV", purchaseDate: "2024-01-01", warrantyMonths: 12, category: "Electronics" }));
	...
	it("handles end-of-month: Jan 31 + 1 month → Feb 29 (2024 leap year)", async () => {
		...
		await POST(makeRequest({ productName: "TV", purchaseDate: "2024-01-31", warrantyMonths: 1, category: "Electronics" }));
		const call = vi.mocked(db.warranty.create).mock.calls[0][0];
	...
	it("handles end-of-month: Jan 31 + 1 month → Feb 28 (2023 non-leap year)", async () => {
		vi.mocked(db.warranty.create).mockClear();
		await POST(makeRequest({ productName: "TV", purchaseDate: "2023-01-31", warrantyMonths: 1, category: "Electronics" }));
		const call = vi.mocked(db.warranty.create).mock.calls[0][0];
		expect(call.data.expiryDate).toEqual(new Date("2023-02-28"));
	});

	it("returns 400 when warrantyMonths is 0", async () => {
		const res = await POST(makeRequest({ productName: "TV", purchaseDate: "2024-01-01", warrantyMonths: 0, category: "Electronics" }));
		expect(res.status).toBe(400);
	});

	it("returns 400 when productName is missing", async () => {
		const res = await POST(makeRequest({ purchaseDate: "2024-01-01", warrantyMonths: 12, category: "Electronics" }));
		expect(res.status).toBe(400);
	});

	it("returns 400 when purchaseDate format is invalid", async () => {
		const res = await POST(makeRequest({ productName: "TV", purchaseDate: "01-01-2024", warrantyMonths: 12, category: "Electronics" }));
		expect(res.status).toBe(400);
	});
});
